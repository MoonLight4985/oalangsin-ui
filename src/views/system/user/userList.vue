    ...
    >
      <div slot="content">
        ...
        >
          ...
          <el-form-item prop="password" v-if="user.uid === ''" label="密码">
          ...
          <el-form-item label="头像">
            <el-upload
              ...
              :data="uploadHeader"
              action="http://localhost:8881/api/oss/file/upload?module=avatar"
            >
...
export default {
  ...
  methods: {
     ...
     * 查询用户列表
     */
    async search(deptId, pageNo = 1, pageSize = 10) {
      this.searchModel.pageNo = pageNo;
      this.searchModel.pageSize = pageSize;
      this.searchModel.deptId = deptId;
      //发送查询请求
    ...
    handleSizeChange(size) {
      ...
      this.search(this.deptId, this.pageNo, size);
    },
    /**
     * 当页码发生变化时触发该事件
     */
    handleCurrentChange(page) {
      this.pageNo = page;
      //调用查询方法
      this.search(this.deptId, page, this.pageSize);
    },
    /**
     * 重置查询条件
     */
    ...
    handleSelectionChange(rows) {
      ...
      // this.selectedIds = rows.map(item => item.uid);//等同于上述代码
    },
    /**
     * 分配角色确认事件
     */
    async onAssignConfirm() {
      //判断用户是否有选中角色
      console.log(this.selectedIds.length + "嘿嘿" + this.selectedIds);
      ...
      //发送请求
      console.log(params.roleIds + "hehe");
      console.log(this.selectedUserId + "哈哈哈哈哈" + this.selectedIds);
